...
use filesystem::{File, FileType, Dir, DirEntry};
use io;
...
impl Seek for DeviceFile
{
    ...
    {
        Err(io::Error { kind: io::ErrorKind::InvalidInput,
...
impl DeviceDir
{
    ...
    {
        DeviceDir { devices: BTreeMap::new() }
    }
...
impl Dir for DeviceDir
{
    fn list_entries(&mut self) -> Vec<DirEntry>
    {
        ...
        self.devices.keys().map(|x| DirEntry{ name: x.clone(),
            typ: FileType::CharacterDevice, size: 0 }).collect()
    }
    ...
    fn get_file(&mut self, name: &str) -> io::Result<Box<File>>
    {
        match self.devices.get(name)
        {
            ...
            Some(dev) => Ok(Box::new(DeviceFile { device: dev.clone() }))
        }
    }
