...
impl<T> SparseVec<T>
{
    ...
    {
        SparseVec { first_empty: 0, array: Vec::new() }
    }

    pub fn with_capacity(capacity: usize) -> SparseVec<T>
    {
        SparseVec { first_empty: 0, array: Vec::with_capacity(capacity) }
    }
    ...
    {
        if index >= self.array.len() { return false; }
    ...
    {
        if index >= self.array.len() { return None; }
        match self.array[index]
        {
