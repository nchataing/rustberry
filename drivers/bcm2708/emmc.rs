...
impl fmt::Debug for CmdError
{
    ...
    {
        write!(f, "{}CMD{} recieved {:?} (instead of {:?})",
               if self.issued_command.contains(SdCmd::IS_APP_CMD) { "A" }
               else { "" }, self.issued_command.bits() >> 24 & 0x7F,
               self.error_irpts, self.expected_irpt)
    }
}
...
{
    fn from(err: CmdError) -> SdError { SdError::CommandError(err) }
}
...
pub fn init() -> Result<SdCard, SdError>
{
    ...
    unsafe
    {
        ...
        if timeout_wait_while!(mmio::read(EMMC_CONTROL1) & (0b111 << 24) != 0,
                              10_000_000)
        {
        ...
        if timeout_wait_while!(mmio::read(EMMC_CONTROL1) & (1 << 1) == 0,
                               10_000_000)
        {
        ...
        loop
        {
            ...
            let op_cond = issue_simple_acmd(SdCmd::SD_SEND_OP_COND,
                                     0x00FF_8000 | v2flags, 0)?.r48;
        ...
        if supports_1_8v {  voltage_switch()?; }
        ...
        let cmd7_resp = issue_busy_cmd(SdCmd::SELECT_DESELECT_CARD,
                                       (rca as u32) << 16)?.r48;
        ...
        let sd_version;
        if sd_spec == 0 { sd_version = SdVersion::Ver1 }
        else if sd_spec == 1 { sd_version = SdVersion::Ver1_1 }
        else if sd_spec == 2
        {
            if sd_spec3 == 0 { sd_version = SdVersion::Ver2 }
            else
            {
                if sd_spec4 == 0 { sd_version = SdVersion::Ver3 }
                else { sd_version = SdVersion::Ver4 }
            }
        }
        else { sd_version = SdVersion::Unknown }
        ...
        Ok(SdCard { is_sdhc, use_1_8v: supports_1_8v, ocr, cid, rca,
                    sd_version, bus_widths })
    }
}
...
unsafe fn issue_cmd_base(cmd: SdCmd, arg: u32) -> Result<CmdResponse, CmdError>
{
    ...
    wait_interrupt(SdInterrupts::COMMAND_COMPLETE)
        .map_err(|e| CmdError { issued_command: cmd, ..e })?;

    match cmd & SdCmd::RSPNS_TYPE_MASK
    {
        SdCmd::RSPNS_TYPE_48 | SdCmd::RSPNS_TYPE_48B =>
            Ok(CmdResponse { r48: mmio::read(EMMC_RESP0) }),
        SdCmd::RSPNS_TYPE_136 =>
            Ok(CmdResponse {
                r136: [mmio::read(EMMC_RESP0), mmio::read(EMMC_RESP1),
                       mmio::read(EMMC_RESP2), mmio::read(EMMC_RESP3)] }),
        _ =>
            Ok(CmdResponse { empty: () })
    }
}
...
unsafe fn issue_read_cmd(cmd: SdCmd, arg: u32, buffer: &mut [u32],
                         block_size: usize) -> Result<CmdResponse, CmdError>
{
    ...
    {
        wait_interrupt(SdInterrupts::BUFFER_READ_READY)
            .map_err(|e| CmdError { issued_command: cmd, ..e })?;
...
unsafe fn issue_write_cmd(cmd: SdCmd, arg: u32, buffer: &[u32],
                          block_size: usize) -> Result<CmdResponse, CmdError>
{
    ...
    {
        wait_interrupt(SdInterrupts::BUFFER_WRITE_READY)
            .map_err(|e| CmdError { issued_command: cmd, ..e })?;
...
unsafe fn issue_simple_acmd(cmd: SdCmd, arg: u32, rca: u16)
    -> Result<CmdResponse, CmdError>
{
    ...
    issue_cmd_base(cmd & !(SdCmd::IS_APP_CMD), arg)
        .map_err(|e| CmdError { issued_command: cmd, ..e })
}

unsafe fn issue_read_acmd(cmd: SdCmd, arg: u32, rca: u16, buffer: &mut [u32],
                         block_size: usize) -> Result<CmdResponse, CmdError>
{
    assert!(cmd.contains(SdCmd::IS_APP_CMD));
    ...
    clean_interrupts();
    issue_cmd_base(SdCmd::APP_CMD, (rca as u32) << 16)?;
    issue_read_cmd(cmd & !(SdCmd::IS_APP_CMD), arg, buffer, block_size)
        .map_err(|e| CmdError { issued_command: cmd, ..e })
}
...
unsafe fn wait_interrupt(wanted_irpt: SdInterrupts) -> Result<(), CmdError>
{
    ...
    if masked_irpts == wanted_irpt { Ok(()) }
    else
    {
        Err(CmdError { issued_command: SdCmd::empty(),
...
unsafe fn wait_for_transfer_complete() -> Result<(), CmdError>
{
    ...
    else
    {
        ...
        {
            Ok(()) => (),
            Err(ref e) if e.error_irpts == SdInterrupts::TRANSFER_COMPLETE
                                         | SdInterrupts::ERR_DATA_TIMEOUT => (),
            Err(e) => return Err(e)
        }
    }
...
impl SdCard
{
    ...
    {
        let status = issue_simple_cmd(SdCmd::SEND_STATUS,
                                      (self.rca as u32) << 16)?.r48;
        let cur_state = (status >> 9) & 0xf;

        if cur_state == 3
        {
            // Currently in the stand-by state - select it
            issue_simple_cmd(SdCmd::SELECT_DESELECT_CARD,
                             (self.rca as u32) << 16)?;
        }
        else if cur_state == 5
        {
        ...
        if cur_state != 4
        {
            let new_status = issue_simple_cmd(SdCmd::SEND_STATUS,
                                          (self.rca as u32) << 16)?.r48;
    ...
    pub fn read(&self, data: &mut [u8], block_id: usize) -> Result<(), SdError>
    {
        ...
        let command;
        if data.len() == BLOCK_SIZE { command = SdCmd::READ_SINGLE_BLOCK; }
        else { command = SdCmd::READ_MULTIPLE_BLOCK; }
        ...
        unsafe
        {
            ...
            let buffer = ::core::slice::from_raw_parts_mut(
                            data.as_mut_ptr() as *mut u32, data.len()/4);
    ...
    pub fn write(&self, data: &[u8], block_id: usize) -> Result<(), SdError>
    {
        ...
        let command;
        if data.len() == BLOCK_SIZE { command = SdCmd::WRITE_BLOCK; }
        else { command = SdCmd::WRITE_MULTIPLE_BLOCK; }
        ...
        unsafe
        {
            ...
            let buffer = ::core::slice::from_raw_parts(
                            data.as_ptr() as *const u32, data.len()/4);
