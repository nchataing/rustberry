...
const ALL_INT : u32 = CTSM_INT | RX_INT | TX_INT | RCV_TIMEOUT_INT
                      | FRAMING_ERROR_INT | PARITY_ERROR_INT | BREAK_ERROR_INT
                      | OVERRUN_ERROR_INT;
...
pub fn has_char_available() -> bool
{
    unsafe
    {
        mmio::read(UART0_FR) & FR_RX_FIFO_EMPTY == 0
    }
}
...
pub fn got_overrun() -> bool
{
    unsafe
    {
        mmio::read(UART0_RSRECR) & RSRECR_OVERRUN != 0
    }
}
...
pub fn flush()
{
    unsafe
    {
        while mmio::read(UART0_FR) & FR_BUSY != 0 {}
    }
}
